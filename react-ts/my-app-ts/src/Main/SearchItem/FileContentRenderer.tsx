const renderFileContent = (item: any) => {
    switch (item.fileType) {
      case 'jpeg':
      case 'png':
      case 'gif':
      case 'svg+xml':
        return <img src={item.file} alt="アップロードされた画像" style={{ maxWidth: '100%', height: 'auto', width: '75%', margin: '0 auto' }} />;
      case 'pdf':
        return <iframe title="PDF Viewer" src={item.file} style={{ width: '80%', height: 'auto', margin: '0 auto' }} />;
      case 'txt':
        return <pre style={{ maxWidth: '100%', margin: '0 auto' }}>{item.content}</pre>;
    　case 'msword':
        return (
            <iframe
            title="Word Viewer"
            src={`https://docs.google.com/gview?url=${encodeURIComponent(item.file)}&embedded=true`}
            style={{ width: 'auto', height: '75%', margin: '0 auto' }}
            />
        );
      case 'mp3':
        return <audio controls src={item.file} style={{ margin: '0 auto', display: 'block', width: '70%' }} />;
      case 'mp4':
        return <video controls width="60%" height="auto" src={item.file} style={{ margin: '0 auto', display: 'block' }} />;
      case 'vnd.ms-powerpoint':
      case 'vnd.ms-excel':
        return (
          <iframe
            title={item.fileType === 'vnd.ms-powerpoint' ? 'PowerPoint Viewer' : 'Excel Viewer'}
            src={`https://view.officeapps.live.com/op/embed.aspx?src=${encodeURIComponent(item.file)}`}
            style={{ width: '70%', height: '500px', margin: '0 auto' }}
          />
        );
      case '':
        return null;
      default:
        return (
          <a href={item.file} target="_blank" rel="noopener noreferrer" style={{ display: 'block', textAlign: 'center' }}>
            ファイルをダウンロード
          </a>
        );
    }
  };
  
  export { renderFileContent };
  